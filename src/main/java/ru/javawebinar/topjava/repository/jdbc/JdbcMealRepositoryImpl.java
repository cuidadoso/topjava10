...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal get(int id, int userId) {
        List<Meal> meals = jdbcTemplate.query("SELECT * FROM meals WHERE id=? AND user_id=?", new MealRowMapper(), id, userId);
    ...
    private Stream<Meal> getAllAsStream(int userId) {
        List<Meal> meals = jdbcTemplate.query("SELECT * FROM meals WHERE user_id=?", new MealRowMapper(), userId);
        return meals == null ?
                Stream.empty() : meals.stream().sorted(MEAL_COMPARATOR);
    }
}
