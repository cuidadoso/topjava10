...
public class UserMealsUtil
{
    ...
    private static final List<UserMeal> MEAL_LIST = Arrays.asList(
    ...
    {
        Map<LocalDate, Integer> daylyCalories =
                mealList.stream().collect(Collectors.groupingBy(um -> um.getDateTime().toLocalDate(),
                        Collectors.summingInt(UserMeal::getCalories)));

        return mealList.stream()
                .filter(um -> TimeUtil.isBetween(um.getDateTime().toLocalTime(), startTime, endTime))
                .map(um -> createMealWithExceded(um, daylyCalories.get(um.getDateTime().toLocalDate()) > caloriesPerDay))
                .collect(Collectors.toList());
    }

    private static UserMealWithExceed createMealWithExceded(final UserMeal userMeal, final boolean exceeded)
    {
